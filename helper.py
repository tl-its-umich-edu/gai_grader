...
#  Reads CSV data files (assignments, gradings, rubrics) and returns a DataFrame containing the required information for processing.
...
# builds a prompt for the grading task using various variables and configuration settings.
...
# Builds the criteria and rubric prompt text based on the provided DataFrame.
...
# Retrieves the student submission text from a file based on the provided assignment and user ID.
...
# Checks if the output for a particular assignment and user ID has already been saved.
...
# Processes the DataFrame row and returns a DataFrame containing the criteria and rubric for the specific row.
...
# Used only to generate the full prompt for the grading task without calling ChatGPT. Used to make templates, for token count debugging, and testing.
def processTokenCount(row, config):        
    ...
    if studentSubmission:
        promptVariableDict = {
                            'Course Name': config.coursePromptNameLookup.get(config.courseShorthand, config.courseShorthand),
...
# processes a single row of data, generates the prompt, and sends it to ChatGPT for grading.
...
#  Checks if the grading task for a specific row needs to be run and saved.
...
# TSimilar to checkRunSave, but it works in multi-processing mode.
...
# Saves the output (grading results) as a pickle file.
...
# Converts the pickle files to a DataFrame.
...
# Checks if a folder exists and contains data.
...
# Defines the configuration parameters for the script.
class Config:
    def __init__(self):
        ...
        self.coursePromptNameLookup: dict = {
                                'MOVESCI': 'Movement Science', \
                                'ECON': 'Economics', \
                                }
        
        self.versionControl: str = None
        self.promptVersion: str = None
        self.courseName: str = None
        self.courseShorthand: str = None
        ...
        self.customDescMode: bool = False
        self.fullName: str = None
        self.critDescDF: str = None
        self.poolSize: int = 8

    # Checks whether the required data folders exist and whether they contain the necessary data.
    ...
    #  Retrieves environment variables and returns their values. It takes the environment variable name as input and, if provided, it casts the value to the specified type.
    def configFetch(self, name, default=None, casting=None, validation=None):
    ...
    # Retrieves configuration parameters from environment variables, sets up logging levels, and performs various checks on the configurations to ensure that they are valid.
    def setFromEnv(self):
        try:
        ...
        self.versionControl = self.configFetch(
            'VERSION_CONTROL', self.versionControl, str)
        envImportSuccess = False if not self.versionControl or not envImportSuccess else True

        self.promptVersion = self.configFetch(
            'PROMPT_VERSION', self.promptVersion, str)
        envImportSuccess = False if not self.promptVersion or not envImportSuccess else True
        ...
        self.courseShorthand = self.configFetch(
            'COURSE_SHORTHAND', self.courseShorthand, str)
        envImportSuccess = False if not self.courseShorthand or not envImportSuccess else True

        self.overWriteSave = self.configFetch(
            'OVERWRITE_SAVE', self.overWriteSave, lambda x: bool(int(x)))
        envImportSuccess = False if not self.overWriteSave or not envImportSuccess else True

        self.customDescMode = self.configFetch(
            'CUSTOM_DESCRIPTIONS', self.customDescMode, lambda x: bool(int(x)))
        envImportSuccess = False if not self.customDescMode or not envImportSuccess else True
    ...
    # If custom parameters are passed theough the dictionary, sets those to override environment set ones instead.
    # It also reads a CSV file containing criterion descriptions if the customDescMode attribute is set to True.
    def setSaveDetails(self, configParams=None):
        if configParams:
            ...
            self.fullName = f'{self.courseShorthand}-{self.versionControl}-{self.promptVersion}'
    ...
    # Generates a template prompt using the promptBuilder() function and saves it to a file.
...
# Represents the OpenAI GPT-3 language model and its functionalities.
class peerGPT:
